export let data = [
  {
      id:"1",
      estado:true,
      titulo: "Cambio de pasta térmica",
      img: "https://cdn-icons-png.flaticon.com/512/8046/8046636.png",
      descripcion: "Descubre cómo la pasta térmica optimiza la transferencia de calor entre el procesador y el disipador, garantizando un rendimiento óptimo al mantener temperaturas adecuadas. Aprende este proceso para desbloquear el potencial de tu procesador y lograr un rendimiento más eficiente en tus tareas y aplicaciones.",
      precio: "Q.100.00",
      link: "https://www.youtube.com/watch?v=Gsm02mt30vU",
      herramientas: [
          "Destornillador de estrella",
          "Paño de microfibra",
          "Pasta térmica"
      ],
      pasos: [
          "Apaga el computador y desconéctalo de la corriente.",
          "Desmonta el disipador de calor del procesador utilizando el destornillador de estrella.",
          "Limpia cuidadosamente la pasta térmica antigua tanto del procesador como del disipador utilizando un paño de microfibra y alcohol isopropílico si es necesario.",
          "Aplica una pequeña cantidad de pasta térmica nueva sobre el procesador.",
          "Vuelve a montar el disipador de calor en su lugar y asegúralo con los tornillos.",
          "Enciende el computador y verifica la temperatura del procesador para asegurarte de que la pasta térmica esté funcionando adecuadamente."
      ]
  },
  {
    id:"2",
      titulo: "Limpieza de los ventiladores",
      estado:true,
      img: "https://img.freepik.com/iconos-gratis/ventilador_318-701983.jpg?w=2000",
      descripcion: "Descubre la relevancia de limpiar los ventiladores de tu computadora para mejorar la refrigeración y el rendimiento. Domina esta técnica para prolongar la vida útil de tu equipo y lograr un desempeño óptimo en todas tus actividades.",
      precio: "Q.100.00",
      link: "https://www.youtube.com/watch?v=i2l0A2dlgGg",
      herramientas: [
          "Destornillador de estrella",
          "Lata de aire comprimido",
          "Alcohol isopropílico",
          "Paño de microfibra"
      ],
      pasos: [
          "Apaga el computador y desconéctalo de la corriente.",
          "Desmonta los ventiladores utilizando el destornillador de estrella.",
          "Utiliza el aire comprimido para soplar el polvo y la suciedad acumulada en los ventiladores.",
          "Limpia las aspas y otras partes de los ventiladores con un paño de microfibra humedecido con alcohol isopropílico.",
          "Vuelve a montar los ventiladores en su lugar y asegúralos con los tornillos.",
          "Enciende el computador y verifica si los ventiladores funcionan correctamente y la refrigeración ha mejorado."
      ]
  },
  {
    id:"3",
      titulo: "Cambio de disco duro a SSD",
      estado:false,
      img: "https://cdn-icons-png.flaticon.com/512/4047/4047694.png",
      descripcion: "Mejora la velocidad y el rendimiento de tu computadora al cambiar el disco duro tradicional por un SSD. Aprende a hacer esta actualización para obtener una mayor capacidad de respuesta y un mejor desempeño en general.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=CxntAJGTiOM",
      herramientas: [
          "Destornillador de estrella",
          "SSD compatible con el computador",
          "Cable SATA (opcional, dependiendo del computador)",
          "Paño de microfibra"
      ],
      pasos: [
          "Apaga el computador y desconéctalo de la corriente.",
          "Abre la carcasa del computador para acceder al disco duro actual.",
          "Desconecta el disco duro y retíralo cuidadosamente.",
          "Conecta el SSD compatible en su lugar.",
          "Si es necesario, conecta el SSD con el cable SATA.",
          "Vuelve a cerrar la carcasa del computador.",
          "Enciende el computador y verifica si el nuevo SSD está funcionando correctamente."
      ]
  },

  {
    id:"4",
    titulo: "Cambio de memoria RAM",
    estado:false,
    img: "https://cdn-icons-png.flaticon.com/512/908/908522.png",
    descripcion: "Aprende cómo esta mejora incrementa la capacidad multitarea y agiliza la ejecución de aplicaciones. Domina este proceso para disfrutar de un desempeño más fluido y eficiente en tus tareas y programas.",
    precio: "Q.100.00",
    link:"https://www.youtube.com/watch?v=0hJRz3bFtSI",
    herramientas: [
      "Destornillador de estrella",
      "Nuevos módulos de memoria RAM compatibles",
      "Paño de microfibra"
    ],
    pasos: [
     " Apaga y desconecta el computador.",
      "Ubica y abre el compartimento de la memoria RAM.",
      "Inserta los nuevos módulos de memoria RAM en las ranuras libres.",
      "Vuelve a cerrar el compartimento y enciende el computador.",
      "Verifica que la nueva memoria RAM sea reconocida por el sistema y funcione correctamente."
    ]
  },
  
    
      {
        id:"5",
        titulo: "Limpieza de una computadora",
        estado:true,
        img:"https://cdn-icons-png.flaticon.com/512/1606/1606145.png",
        descripcion: "Aprende a limpiar tu computadora de manera segura y efectiva para mantenerla en condiciones óptimas de funcionamiento.Domina esta técnica para garantizar un óptimo funcionamiento y una mayor durabilidad de tu computadora.",
        precio: "Q.100.00",
        link:"https://www.youtube.com/watch?v=OKlMJ5eJnJQ",
        herramientas: [
          "Paño de microfibra",
          "Aire comprimido",
          "Alcohol isopropílico",
          "Hisopos de algodón",
          "Destornillador (si es necesario para acceder a componentes internos)"
        ],
        pasos: [
         " Colocar el computador boca abajo en una superficie plana.",
         " Limpiar suavemente la superficie externa con un paño de microfibra.",
         " Utilizar aire comprimido para limpiar los ventiladores y aberturas.",
          "Limpiar áreas de difícil acceso con hisopos humedecidos en alcohol isopropílico.",
      "    Volver a colocar la cubierta posterior y asegurar los tornillos.",
         " Encender el computador y verificar su correcto funcionamiento.",
      "    Realizar limpiezas regulares para mantener el computador en óptimas condiciones.    "    ]
      },
    {
      id:"6",
      titulo: "Solución de problemas de conexión Wi-Fi",
      estado:true,
      img: "https://cdn-icons-png.flaticon.com/512/1126/1126411.png",
      descripcion:  "Accede a esta guía para resolver problemas de conectividad Wi-Fi en tus dispositivos. Encuentra soluciones efectivas que te ayudarán a restablecer la conexión y disfrutar de una experiencia fluida en línea.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=8crHCIHxCFw",
      herramientas: ["Computadora", "Router", "Comprobador de señal Wi-Fi"],
      pasos: [
        "Reiniciar el dispositivo con problemas de conexión Wi-Fi.",
        "Reiniciar el router y esperar a que se restablezca la conexión.",
        "Asegurarse de que el dispositivo esté dentro del alcance del router y sin obstrucciones.",
        "Verificar si otros dispositivos tienen problemas similares de conexión Wi-Fi.",
        "Comprobar si hay actualizaciones de firmware disponibles para el router.",
        "Cambiar el canal del router para evitar interferencias.",
        "Restablecer la configuración de red en el dispositivo con problemas.",
        "Desconectar y volver a conectar el Wi-Fi en el dispositivo.",
        "Verificar la configuración de seguridad del router (contraseña, filtro MAC, etc.).",
        "Si los problemas persisten, considerar contactar al proveedor de servicios de Internet o al soporte técnico del dispositivo o router."
      ]
    },    
    {
      id:"7",
      titulo: "Mantenimiento de computadora",
      estado:false,
      img: "https://cdn-icons-png.flaticon.com/512/2920/2920288.png",
      descripcion: "Descubre valiosos consejos de mantenimiento para potenciar el rendimiento y la durabilidad de tu computadora a través de prácticas regulares.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=VK4MquWx_a0",
      herramientas: ["Paño de microfibra", "Aire comprimido", "Destornillador"],
      pasos: [
        "Apagar y desconectar la computadora.",
        "Limpiar suavemente la superficie externa con un paño de microfibra.",
       " Usar aire comprimido para limpiar los componentes internos.",
    "    Verificar y reemplazar los ventiladores desgastados o ruidosos.",
        "Inspeccionar las conexiones internas y asegurarse de que estén bien conectadas.",
        "Actualizar el software, incluyendo sistema operativo y controladores.",
        "Instalar y mantener actualizado el software antivirus y antimalware.",
        "Cerrar la cubierta y conectar la computadora a la fuente de energía.",
       " Encender la computadora y verificar su funcionamiento general después del mantenimiento."
                   
      ]
    },
    {
      id:"8",
      titulo: "Recuperación de datos borrados",
      estado:true,
      img: "https://www.profesionalreview.com/wp-content/uploads/2018/02/programas-para-recuperar.jpg",
      descripcion: "Aprende los pasos necesarios para recuperar archivos accidentalmente eliminados en una unidad de almacenamiento, permitiéndote restaurar información valiosa de manera efectiva.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=F9vAQLBW46c",
      herramientas: ["Software de recuperación de datos", "Conexión a una computadora"],
      pasos: [
        "Dejar de usar la unidad de almacenamiento inmediatamente.",
        "Conectar la unidad a una computadora y descargar un software de recuperación.",
       " Ejecutar el software y escanear la unidad para buscar archivos borrados.",
        "Seleccionar y guardar los archivos recuperados en una ubicación segura.",
        "Evitar guardarlos nuevamente en la misma unidad.",
      "  Verificar que los archivos se abren correctamente.",
       " Realizar copias de seguridad regulares para evitar futuras pérdidas de datos."
      ]
    },
    {
      id:"9",
      titulo: "Reparación de teclado atascado",
      estado:true,
      img: "https://previews.123rf.com/images/madmaxer/madmaxer1105/madmaxer110500080/9518823-ilustraci%C3%B3n-3d-de-teclado-de-ordenador-con-herramientas-manuales-concepto-de-reparaci%C3%B3n-de-equipo.jpg",
      descripcion: "Aprende cómo solucionar teclas atascadas en un teclado de computadora con estas instrucciones prácticas y recupera su funcionalidad sin problemas.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=2BHxley0O0w",
      herramientas: ["Destornillador", "Toallitas de limpieza", "Pincel"],
      pasos: [
        "Apagar y desconectar la computadora.",
        "Limpiar suavemente las teclas y los espacios entre ellas con toallitas o paño humedecido en alcohol isopropílico.",
        "Verificar y limpiar residuos de líquidos derramados.",
       " Reemplazar las teclas si es necesario o buscar asistencia técnica profesional si el problema persiste."]
    },
    
    {
      id:"10",
      titulo: "Instalación de actualizaciones del sistema operativo",
      estado:true,
      img: "https://zeus.descubrecomohacerlo.com/uploads/images/content/laptop-seguridad-herramienta_2761.jpg",
      descripcion: "Descubre los pasos esenciales para instalar las últimas actualizaciones de seguridad y funciones en tu sistema operativo, manteniendo tu sistema protegido y aprovechando las mejoras más recientes.",
      precio: "Q.100.00",
      link:"https://www.youtube.com/watch?v=zjIOUJMN5ok",
      herramientas: ["Conexión a Internet", "Espacio de almacenamiento suficiente"],
      pasos: [
        "Guardar y cerrar todos los archivos y programas abiertos",
        "Conectar la computadora a una fuente de alimentación o asegurarse de tener suficiente batería",
        "Verificar una conexión a Internet estable y rápida",
        "Buscar la sección de 'Actualizaciones' o 'Actualización y seguridad'",
        "Iniciar el proceso de búsqueda de actualizaciones",
        "Descargar e instalar las actualizaciones recomendadas",
        "Esperar a que se complete la instalación de las actualizaciones",
        "Reiniciar la computadora si es necesario",
        "Verificar nuevamente la sección de actualizaciones",
        "Realizar una prueba de funcionalidad básica",
        "Configurar nuevas funciones o ajustes si es necesario",
        "Mantener activadas las actualizaciones automáticas"
              ]
    }
];
